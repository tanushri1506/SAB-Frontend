...
const fetchCouncil = async () => {
  const res = await fetch(`${API_BASE}/api/council/?tenure=2025-26`, { cache: "no-store" });
...
export const useContacts = () => {
  ...
  useEffect(() => { fetchContacts().then(setContacts).catch(console.error); }, []);
  return contacts;
};

export const useCouncil = () => {
  const [council, setCouncil] = useState([]);
  useEffect(() => { fetchCouncil().then(setCouncil).catch(console.error); }, []);
  return council;
};

export const useEvents = () => {
  const [events, setEvents] = useState([]);
  useEffect(() => { fetchEvents().then(setEvents).catch(console.error); }, []);
  return events;
};

export const usePhotos = () => {
  const [photos, setPhotos] = useState([]);
  useEffect(() => { fetchPhotos().then(setPhotos).catch(console.error); }, []);
  return photos;
};
