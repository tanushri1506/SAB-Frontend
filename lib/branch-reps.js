...
//   const res = await fetch(`${API_BASE}/api/branch-reps/?tenure=${tenure}`, { cache: "no-store" });
//   if (!res.ok) throw new Error("Failed to fetch branch representatives");
//   return res.json();
// };

// // Branch Representatives with tenure
// export const BRANCH_REPS = await getBranchReps().then((team) =>
//   team.map((m) => ({
//     name: m.name || "",
//     post: m.post || "",
//     tenure: m.tenure || "2024-28", // default tenure
//     phone: m.phone || "",
//     email: m.email || "",
//     linkedin: m.linkedin || "",
//     Photo: (
//       <Image
//         src={m.photo?.replace('http://', 'https://')}
//         alt={m.name}
//         width={200}
//         height={200}
...
export const getBranchReps = async (tenure = "2024-28") => {
  ...
  return data.map((m) => ({
    ...
    Photo: m.photo ? (
      <Image
        src={m.photo.replace("http://", "https://")} // ensure HTTPS
        alt={m.name}
        width={200}
        height={200}
        className="object-cover"
